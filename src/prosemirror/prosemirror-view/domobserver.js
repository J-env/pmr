...
export class DOMObserver {
  constructor(view) {
    this.view = view;
    this.observer = window.MutationObserver && new window.MutationObserver(mutations => this.registerMutations(mutations));

    if (useCharData) {
      this.onCharData = e => this.registerMutation({
        target: e.target,
        type: 'characterData',
  ...
  registerMutation(mut) {
    ...
    if (mut.type === 'childList') {
      let fromOffset = mut.previousSibling && mut.previousSibling.parentNode === mut.target ? domIndex(mut.previousSibling) + 1 : 0;
      if (fromOffset == -1) return;
      from = desc.localPosFromDOM(mut.target, fromOffset, -1);

      let toOffset = mut.nextSibling && mut.nextSibling.parentNode === mut.target ? domIndex(mut.nextSibling) : mut.target.childNodes.length;
