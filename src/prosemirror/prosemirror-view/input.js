...
editHandlers.keypress = (view, event) => {
  if (view.inDOMChange || !event.charCode || event.ctrlKey && !event.altKey || browser.mac && event.metaKey) return;
...
function capturePaste(view, e) {
  ...
  setTimeout(() => {
    ...
    if (plainText) {
      ...
      let html = (target.innerHTML || '').replace(/^([\s\S]*<div>)|(<\/div>[\s\S]*)$/gi, '');
...
// <= ie10 running
if (browser.ie_version != null && browser.ie_version <= 10) {
  editHandlers.beforepaste = (view, e) => {
    e.preventDefault();
    capturePaste(view, e);
  };
}
...
editHandlers.paste = (view, e) => {
  ...
  }else if (browser.ie_version >= 11 || browser.ie_version == null) {
    capturePaste(view, e);
  }
}
...
editHandlers.dragover = editHandlers.dragleave = editHandlers.dragenter = (_, e) => e.preventDefault();
